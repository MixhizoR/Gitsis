...
const FIELDS = [
  'Arayuz / HMI', 'Yazilim / Kontrol', 'Donanim', 'Veritabani',
  'Sunucu / Altyapi', 'Haberlesme', 'Guvenlik / Emniyet', 'Performans', 'Genel',
]
...
const USER_TITLES = [
  'Ucus guvenligi ve emniyet', 'Otonom ucus kabiliyeti', 'Pilot kullanim kolayligi',
  'Ucus verisi kayit ve raporlama', 'Yer istasyonu ile haberlesme', 'Sistem bakim ve teshis',
  'Yetkilendirme ve erisim guvenligi', 'Cevresel dayaniklilik', 'Guc yonetimi ve sureklilik',
  'Standartlara uygunluk (DO-178C)', 'Gorev planlama ve yeniden planlama', 'Sistem genisletilebilirligi',
]
const SYSTEM_TITLES = [
  'Otopilot tepki suresi', 'Irtifa koruma dogrulugu', 'Ariza durumunda guvenli mod',
  'Kritik uyari gosterimi', 'IMU sensor yedekliligi', 'Ucus verisi kaydi ve saklama',
  'Telemetri sunucu servisi', 'Haberlesme veri butunlugu', 'Erisim yetkilendirmesi',
  'Kesintisiz guc ve yeniden baslatma', 'Rota takip dogrulugu', 'Otomatik inis destegi',
  'Hava durumu verisi entegrasyonu', 'Gorev yukleme ve dogrulama', 'Sistem oz-teshis (BIT)',
  'Zaman senkronizasyonu (GPS)', 'Cift kanal aktif-yedek mimari', 'Ivme ve titresim toleransi',
  'Konfigurasyon surum yonetimi', 'Kayit disi olay gunlugu',
]
const SW_TITLES = [
  'PID kontrol dongusu', 'Sensor fuzyon algoritmasi', 'Irtifa kestirim filtresi',
  'Guvenli mod gecis mantigi', 'Uyari onceliklendirme motoru', 'HMI ekran yenileme yonetimi',
  'Telemetri paketleme servisi', 'CRC dogrulama modulu', 'Rol tabanli yetki denetimi',
  'Guc yonetim durum makinesi', 'Rota takip denetleyicisi', 'Otomatik inis profili',
  'Hava durumu ayristirici', 'Gorev dosyasi dogrulayici', 'BIT test yuruttucu',
  'GPS zaman senkron servisi', 'Aktif-yedek gecis yoneticisi', 'Veri kayit tampon yoneticisi',
  'Konfigurasyon yukleyici', 'Olay gunlugu yazici', 'Komut dogrulama katmani',
  'Watchdog denetleyici', 'Parametre sinir denetimi', 'Hata kurtarma yordami',
]
const HW_TITLES = [
  'Birincil IMU karti', 'Yedek IMU karti', 'Guc dagitim karti',
  'Yedek batarya modulu', 'Ana islemci karti (FCC)', 'Haberlesme arayuz karti',
  'GPS alici modulu', 'Analog giris karti', 'Aktuator surucu karti',
  'Kalici bellek (flash) modulu', 'Sicaklik sensoru dizisi', 'Titresim sonumleyici montaj',
  'Pilot gosterge paneli', 'Sesli alarm birimi', 'Harici veri portu (ARINC)',
  'Yalitim / EMI koruma modulu',
]

// --- Test senaryosu basliklari ----------------------------------------------
const ACC_TITLES = [
  'Ucus guvenligi kabul testi', 'Otonom ucus kabul testi', 'Pilot arayuz kullanilabilirlik testi',
  'Veri kayit kabul testi', 'Yer haberlesmesi kabul testi',
]
const SYS_TEST_TITLES = [
  'Otopilot tepki suresi testi', 'Irtifa koruma testi', 'Guvenli mod gecis testi',
  'Kritik uyari gosterim testi', 'IMU yedeklilik testi', 'Kesintisiz guc testi',
]
const SUB_TEST_TITLES = [
  'PID kontrol birim testi', 'Sensor fuzyon birim testi', 'CRC dogrulama birim testi',
  'Birincil IMU donanim testi', 'Guc dagitim donanim testi',
]
...
async function main() {
  ...
  const TEST_STATUS = {
    'TC-ACC-001': STATUS.APPROVED, 'TC-ACC-002': STATUS.APPROVED, 'TC-ACC-003': STATUS.IN_REVIEW,
    'TC-ACC-004': STATUS.REJECTED, 'TC-ACC-005': STATUS.APPROVED,
    'TC-SYS-001': STATUS.APPROVED, 'TC-SYS-002': STATUS.APPROVED, 'TC-SYS-003': STATUS.APPROVED,
    'TC-SYS-004': STATUS.IN_REVIEW, 'TC-SYS-005': STATUS.REJECTED, 'TC-SYS-006': STATUS.APPROVED,
    'TC-SUB-001': STATUS.APPROVED, 'TC-SUB-002': STATUS.APPROVED, 'TC-SUB-003': STATUS.IN_REVIEW,
    'TC-SUB-004': STATUS.APPROVED, 'TC-SUB-005': STATUS.REJECTED,
  }
  const testData = []
  const pushTest = (type, prefix, i, title) => {
    const text_id = `${prefix}-${pad(i)}`
    testData.push({
      projectId: pid, text_id, title,
      description: `${title} — dogrulama senaryosu.`,
      type,
      field: null, priority: null, dal_level: null,
  ...
  const verifyMap = [
    ['REQ-USR-001', 'TC-ACC-001'], ['REQ-USR-002', 'TC-ACC-002'], ['REQ-USR-003', 'TC-ACC-003'],
    ['REQ-USR-004', 'TC-ACC-004'], ['REQ-USR-005', 'TC-ACC-005'],
    ['REQ-SYS-001', 'TC-SYS-001'], ['REQ-SYS-002', 'TC-SYS-002'], ['REQ-SYS-003', 'TC-SYS-003'],
    ['REQ-SYS-004', 'TC-SYS-004'], ['REQ-SYS-005', 'TC-SYS-005'], ['REQ-SYS-006', 'TC-SYS-006'],
    ['REQ-SW-001', 'TC-SUB-001'], ['REQ-SW-002', 'TC-SUB-002'], ['REQ-SW-003', 'TC-SUB-003'],
    ['REQ-HW-001', 'TC-SUB-004'], ['REQ-HW-002', 'TC-SUB-005'],
  ]
  ...
  if (reqData.length !== 72 || testData.length !== 16 || links.length !== 58) {
    console.warn(
      `[seed] UYARI: beklenen 72/16/58 ile uyumsuz -> ${reqData.length}/${testData.length}/${links.length}`
    )
  }
}
