...
async function cascade(projectId) {
  ...
  for (const c of changes) {
    ...
    await audit(projectId, {
      action: 'AUTO_STATUS', entityType: 'requirement', entityId: c.id, textId: c.text_id,
      field: 'status', oldValue: c.from, newValue: c.to,
...
async function batchDelete(pid, model, ids, entityType) {
  ...
  for (const r of rows) {
    await audit(pid, {
      action: 'DELETE', entityType, entityId: r.id, textId: r.text_id,
...
app.post('/api/auth/register', wrap(async (req, res) => {
  ...
  const user = await prisma.user.create({
    data: { username: username.trim(), password: await hashPassword(password), name: name.trim(), initials, role: role || 'System Engineer' },
  })
...
app.post('/api/auth/login', wrap(async (req, res) => {
  ...
  if (migrated) await prisma.user.update({ where: { id: user.id }, data: { password: await hashPassword(password) } })
  const token = signToken({ kind: 'pm', isPM: true, userId: user.id })
  res.json({ token, user: safeUser(user) })
}))

app.get('/api/users', requirePM, wrap(async (_req, res) => {
...
app.post('/api/auth/passcode', wrap(async (req, res) => {
  ...
  const token = signToken({ kind: 'personnel', isPM: false, personnelId: p.id, projectId: p.projectId, roleId: p.roleId })
  res.json({
    token,
...
app.post('/api/projects', requirePM, wrap(async (req, res) => {
  const { name, description } = req.body || {}
  if (!name || !name.trim()) throw bad('Proje adi zorunlu.')
  const project = await prisma.project.create({
    data: { name: name.trim(), description: (description || '').trim() },
  })
  await audit(project.id, { action: 'PROJECT_CREATE', entityType: 'project', entityId: project.id, message: `Proje olusturuldu: "${project.name}".` })
...
app.patch('/api/projects/:pid', requirePM, wrap(async (req, res) => {
  const { name, description } = req.body || {}
  const data = {}
...
app.delete('/api/projects/:pid', requirePM, wrap(async (req, res) => {
...
// ===========================================================================
app.get('/api/projects/:pid/fields', wrap(async (req, res) => {
  const fields = await prisma.projectField.findMany({ where: { projectId: req.params.pid }, orderBy: { name: 'asc' } })
  res.json(fields)
}))

app.post('/api/projects/:pid/fields', wrap(async (req, res) => {
  const { name } = req.body || {}
...
app.delete('/api/projects/:pid/fields/:id', wrap(async (req, res) => {
...
//  REQUIREMENTS
// ===========================================================================
app.get('/api/projects/:pid/requirements', wrap(async (req, res) => {
...
app.post('/api/projects/:pid/requirements', wrap(async (req, res) => {
  const pid = req.params.pid
  const b = req.body || {}
  ...
  await audit(pid, { action: 'CREATE', entityType: 'requirement', entityId: row.id, textId: row.text_id, message: `Yeni gereksinim: "${row.title}" (${row.type}).` })
  res.status(201).json(row)
}))

app.get('/api/projects/:pid/requirements/:id', wrap(async (req, res) => {
  const row = await prisma.requirement.findUnique({ where: { id: req.params.id } })
  if (!row) throw bad('Gereksinim bulunamadi.', 404)
  res.json(row)
}))

app.put('/api/projects/:pid/requirements/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  const b = req.body || {}
  ...
  await audit(pid, { action: 'UPDATE', entityType: 'requirement', entityId: row.id, textId: row.text_id, message: `Gereksinim guncellendi: "${row.title}".` })
  res.json(row)
}))

app.delete('/api/projects/:pid/requirements/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  const before = await prisma.requirement.findUnique({ where: { id: req.params.id } })
  if (!before || before.projectId !== pid) throw bad('Gereksinim bulunamadi.', 404)
  if (before.locked) throw bad('Bu gereksinim onaylandi ve kilitli; silinemez.', 403)
  // Iliskili baglari temizle
  await prisma.traceabilityLink.deleteMany({ where: { projectId: pid, OR: [{ fromId: req.params.id }, { toId: req.params.id }] } })
  await prisma.requirement.delete({ where: { id: req.params.id } })
  await audit(pid, { action: 'DELETE', entityType: 'requirement', entityId: req.params.id, textId: before.text_id, message: `Gereksinim silindi: "${before.title}".` })
  await cascade(pid)
  res.json({ ok: true })
}))

app.post('/api/projects/:pid/requirements/batch-delete', wrap(async (req, res) => {
  const pid = req.params.pid
...
//  TEST CASES
// ===========================================================================
app.get('/api/projects/:pid/testcases', wrap(async (req, res) => {
  const where = { projectId: req.params.pid }
  if (req.query.type) where.type = req.query.type
...
app.post('/api/projects/:pid/testcases', wrap(async (req, res) => {
  const pid = req.params.pid
  const b = req.body || {}
  ...
  await audit(pid, { action: 'CREATE', entityType: 'testcase', entityId: row.id, textId: row.text_id, message: `Yeni test senaryosu: "${row.title}" (${row.type}).` })
  res.status(201).json(row)
}))

app.put('/api/projects/:pid/testcases/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  const b = req.body || {}
  ...
  await audit(pid, { action: 'UPDATE', entityType: 'testcase', entityId: row.id, textId: row.text_id, message: `Test guncellendi: "${row.title}" (durum: ${row.status}).` })
  // Test durumu degistiyse cascade
  await cascade(pid)
  res.json(row)
}))

app.delete('/api/projects/:pid/testcases/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  const before = await prisma.testCase.findUnique({ where: { id: req.params.id } })
  if (!before || before.projectId !== pid) throw bad('Test bulunamadi.', 404)
  ...
  await audit(pid, { action: 'DELETE', entityType: 'testcase', entityId: req.params.id, textId: before.text_id, message: `Test silindi: "${before.title}".` })
  await cascade(pid)
  res.json({ ok: true })
}))

app.post('/api/projects/:pid/testcases/batch-delete', wrap(async (req, res) => {
  const pid = req.params.pid
...
//  GLOSSARY
// ===========================================================================
app.get('/api/projects/:pid/glossary', wrap(async (req, res) => {
  const rows = await prisma.glossaryTerm.findMany({ where: { projectId: req.params.pid }, orderBy: { term: 'asc' } })
  res.json(rows)
}))

app.post('/api/projects/:pid/glossary', wrap(async (req, res) => {
  const pid = req.params.pid
  const b = req.body || {}
  ...
  const row = await prisma.glossaryTerm.create({
    data: { projectId: pid, text_id, term: b.term.trim(), definition: (b.definition || '').trim(), author: b.author || 'ehsim.user' },
  })
  await audit(pid, { action: 'CREATE', entityType: 'glossary', entityId: row.id, textId: row.text_id, message: `Sozluk terimi eklendi: "${row.term}".` })
  res.status(201).json(row)
}))

app.put('/api/projects/:pid/glossary/:id', wrap(async (req, res) => {
  const pid = req.params.pid
...
app.delete('/api/projects/:pid/glossary/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  const before = await prisma.glossaryTerm.findUnique({ where: { id: req.params.id } })
  if (!before || before.projectId !== pid) throw bad('Terim bulunamadi.', 404)
  await prisma.traceabilityLink.deleteMany({ where: { projectId: pid, OR: [{ fromId: req.params.id }, { toId: req.params.id }] } })
...
app.post('/api/projects/:pid/glossary/batch-delete', wrap(async (req, res) => {
  const pid = req.params.pid
...
//  LINKS (traceability)
// ===========================================================================
app.get('/api/projects/:pid/links', wrap(async (req, res) => {
...
async function createOneLink(pid, { fromId, toId, type, testStatus: _testStatus }) {
  ...
  const link = dup || (await prisma.traceabilityLink.create({
    data: { projectId: pid, fromId, toId, type, createdBy: 'ehsim.user' },
  }))
  if (!dup) {
    await audit(pid, { action: 'LINK', entityType: 'link', entityId: link.id, textId: `${fromR.node.text_id} -> ${toR.node.text_id}`, message: `Bag kuruldu: ${fromR.node.text_id} «${type}» ${toR.node.text_id}.` })
  }
  return link
}
...
app.post('/api/projects/:pid/links/batch', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  for (const sourceId of sourceIds) {
    if (sourceId === targetId) { results.skipped.push({ id: sourceId, reason: 'self' }); continue }
    try {
...
app.delete('/api/projects/:pid/links/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  await audit(pid, { action: 'UNLINK', entityType: 'link', entityId: req.params.id, message: `Bag koparildi (${before.type}).` })
  await cascade(pid)
  res.json({ ok: true })
}))
...
//  AUDIT
// ===========================================================================
app.get('/api/projects/:pid/audit', wrap(async (req, res) => {
  const rows = await prisma.auditLog.findMany({ where: { projectId: req.params.pid }, orderBy: { createdAt: 'desc' }, take: 1000 })
  res.json(rows)
}))

app.post('/api/projects/:pid/audit', wrap(async (req, res) => {
...
// ===========================================================================
app.post('/api/projects/:pid/recompute', wrap(async (req, res) => {
...
// ===========================================================================
app.get('/api/projects/:pid/roles', wrap(async (req, res) => {
  const rows = await prisma.role.findMany({ where: { projectId: req.params.pid }, orderBy: { createdAt: 'asc' } })
  res.json(rows)
}))

app.post('/api/projects/:pid/roles', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  await audit(pid, { action: 'ROLE_CREATE', entityType: 'role', entityId: row.id, message: `Rol olusturuldu: "${row.name}".` })
  res.status(201).json(row)
}))

app.put('/api/projects/:pid/roles/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  await audit(pid, { action: 'ROLE_UPDATE', entityType: 'role', entityId: row.id, message: `Rol guncellendi: "${row.name}".` })
  // Izinler degistiginde onay durumlari etkilenebilir; yeniden hesapla.
  await recomputeAllApprovals(pid)
  res.json(row)
}))

app.delete('/api/projects/:pid/roles/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  const before = await prisma.role.findUnique({ where: { id: req.params.id } })
  if (!before || before.projectId !== pid) throw bad('Rol bulunamadi.', 404)
  await prisma.role.delete({ where: { id: req.params.id } })
  await audit(pid, { action: 'ROLE_DELETE', entityType: 'role', entityId: req.params.id, message: `Rol silindi: "${before?.name || ''}".` })
  await recomputeAllApprovals(pid)
  res.json({ ok: true })
}))
...
// ===========================================================================
app.get('/api/projects/:pid/personnel', wrap(async (req, res) => {
...
app.post('/api/projects/:pid/personnel', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  await audit(pid, { action: 'PERSONNEL_CREATE', entityType: 'personnel', entityId: row.id, message: `Personel eklendi: "${row.firstName} ${row.lastName}" (${role.name}), passcode: ${passcode}.` })
  await recomputeAllApprovals(pid)
  res.status(201).json(row)
}))

app.delete('/api/projects/:pid/personnel/:id', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  await audit(pid, { action: 'PERSONNEL_DELETE', entityType: 'personnel', entityId: req.params.id, message: `Personel silindi: "${before?.firstName || ''} ${before?.lastName || ''}".` })
  await recomputeAllApprovals(pid)
  res.json({ ok: true })
}))
...
app.get('/api/projects/:pid/approvals', wrap(async (req, res) => {
...
app.post('/api/projects/:pid/approvals/vote', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  if (existing) {
    ...
    await audit(pid, { action: 'APPROVAL_WITHDRAW', entityType, entityId, textId: entity.text_id, actor: voterName || voterId, message: `Onay geri cekildi: ${voterName || voterId}.` })
  } else {
    await prisma.approval.create({
      data: { projectId: pid, entityType, entityId, voterId, voterName: voterName || voterId, personnelId: personnelId || null },
    })
    await audit(pid, { action: 'APPROVAL_VOTE', entityType, entityId, textId: entity.text_id, actor: voterName || voterId, message: `Onaylandi: ${voterName || voterId}.` })
  }
...
app.post('/api/projects/:pid/approvals/unlock', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  const model = entityType === 'requirement' ? 'requirement' : 'testCase'
  const entity = await prisma[model].findUnique({ where: { id: entityId } })
  await audit(pid, { action: 'UNLOCK', entityType, entityId, textId: entity?.text_id, actor: 'Proje Yoneticisi', message: 'Kilit acildi; PM onayi geri cekildi, durum Beklemede.' })
  const state = await recomputeApproval(pid, entityType, entityId)
  res.json(state)
}))

// Onay detay matrisi (PM'e ozel): her gerekli oy verenin oy durumu.
app.get('/api/projects/:pid/approvals/matrix', wrap(async (req, res) => {
  const pid = req.params.pid
  ...
  const approvals = await prisma.approval.findMany({ where: { projectId: pid, entityType, entityId: String(entityId) } })
  const votedIds = new Set(approvals.map((a) => a.voterId))
  const voters = [
    { voterId: 'PM', name: 'Proje Yoneticisi', role: 'Proje Yoneticisi', voted: votedIds.has('PM') },
    ...requiredPersonnel.map((p) => ({
      voterId: p.id, name: `${p.firstName} ${p.lastName}`, role: p.role?.name || '-', voted: votedIds.has(p.id),
    })),
  ]
  res.json({ approvalStatus: entity.approvalStatus, locked: entity.locked, textId: entity.text_id, title: entity.title, voters })
}))
